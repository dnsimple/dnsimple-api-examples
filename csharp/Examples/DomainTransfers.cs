...
public static class DomainTransfers
{
    public static void Run(string[] args)
    {
        ...
        Console.WriteLine($"Getting transfer {transferId} for {name}");

        // retrieve the details of an existing domain transfer
        try
        {
            var transfer = client.Registrar.GetDomainTransfer(accountId, name, transferId).Data;
            Console.WriteLine($"Transfer: Id={transfer.Id}, DomainId={transfer.DomainId}, State={transfer.State}, StatusDescription={transfer.StatusDescription}");
        }
        catch (DnsimpleException e)
        {
            Console.Error.WriteLine($"GetDomainTransfer() returned error: {e.Message}");
            Environment.Exit(1);
        }
    }
}
