...
Here's what we'll delve into in this demo:
...
Enforce organizational and regulatory policies instantly with Sentinel’s policy-as-code framework, ensuring every DNS change adheres to the set standards before being applied.

- Version-Controlled Policy Management
Track every policy and DNS change in your version control system, fostering a culture of accountability and providing a clear audit trail.
...
- Two domains registered at DNSimple. Don't have an account? [Sign up free](https://dnsimple.com/sign_up).
...
2. Next, we want to authenticate your local environment with Terraform Cloud. Do this by setting environment variables as follows:
...
Alternatively, you can authenticate with Terraform Cloud by running `terraform login` and following the prompts. If you take this route, add the following to your `main.tf` file:
...
Create a new policy in Terraform Cloud. Copy the contents of `policies.sentinel` into the policy. You will also need to create a policy set and attach the policy to the policy set. When creating the policy set, select the workspace you created in Step 2, otherwise the policies will be applied to all workspaces in your organization.

Fill out the following variables in the policy with your own values:

```hcl
allowed_contact_ids = [10169]
```

HINT: To get the contact ID, run the following command in the Terraform CLI:
...
The policy includes additional variables you can customize. For example, you can customize the enforced state for each of the checks, and which domains or TLDs to skip when running the policy.

You will notice in the policy there are 5 separate rules being enforced. These rules are:
...
You can customize the policy by removing or adding rules.
...
Begin by creating a new GitHub repository. Name it whatever you like, for example,`dnsimple-sentinel-demo`. Once you have created the repository, clone it to your local machine.
...
Commit and push only the following files. The rest of the files will be added in the next step as part of a Pull Request:
...
You successfully completed the "DNSimple Policy-as-Code with Terraform and Sentinel" demo. You now know how to utilize Terraform, Sentinel, and DNSimple to establish a policy-driven DNS management framework, empowering your organization to democratize DNS management while ensuring policy adherence.

When thinking about moving to production and maturing your policy-as-code framework, here are some things to consider:

- **Modularize Policies**: In the current demo, we created the policy manually in Terraform Cloud, however, to make the policy more reusable and easier to maintain, you can create a module for each policy and load them as a policy set in Terraform Cloud through version control. This will allow you to easily reuse the policies across multiple workspaces and organizations and version control the policies with automated release processes and audit trails.
- **Policy Testing**: As your policy-as-code framework matures, you will want to ensure that your policies are working as expected. You can do this by writing tests for your policies. Read more about policy testing in the [Sentinel documentation](https://docs.hashicorp.com/sentinel/terraform/testing).
...
Reach out to us at [support at dnsimple dot com](mailto:support@dnsimple.com) if you have any questions or feedback. We'd love to hear from you.
