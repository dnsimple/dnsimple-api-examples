...
pub fn list_domains(token: &str) {
    ...
    let response = client.domains().list_domains(account_id, None);

    println!("These are your domains:");

    for domain in response.unwrap().data.unwrap() {
        println!("- {:?}", domain)
    }
}
