...
# Grabs the first domain on this account
domain = case Dnsimple.Domains.domains(client, account.id) do
  {:ok, response} -> List.first(response.data)
  {:error, error} -> raise RuntimeError, message: error.message
end

# List all the DNS records for this domain
case Dnsimple.Zones.records(client, account.id, domain.name) do
